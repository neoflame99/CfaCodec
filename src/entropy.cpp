...
/*
    proc_info.dwt_lv;
    int32_t bandidx[MNB];
    for(int32_t t=0; t < MNB; ++t){
        bandidx[k] = -1;
    }
    for(int32_t t=0; t < proc_info.dwt_lv; ++t){
        bandidx[k] = msst_info.w >> t; 
        if(t < proc_info.dwt_lv){
            bandidx[k]= proc_info.dwt_lv-t;
        }else{
            bandidx[k]= -1; 
        }
    }
    ...
    for(int32_t r=0; r < msst_info.h; ++r){
        ...
        pos = 4;
        bp  = 0;
        for(int32_t c=0; c < msst_info.w; c+=4, k+=4){
            ybcnt=0 ; yscnt = 0;
            dgbcnt=0; dgscnt= 0;
            cgbcnt=0; cgscnt= 0;
            cobcnt=0; coscnt= 0;
            my = msstv[k].mY | msstv[k+1].mY | msstv[k+2].mY | msstv[k+3].mY;
            mdg= msstv[k].mYdDg| msstv[k+1].mYdDg| msstv[k+2].mYdDg| msstv[k+3].mYdDg;
            mco= msstv[k].mCrCo| msstv[k+1].mCrCo| msstv[k+2].mCrCo| msstv[k+3].mCrCo;
            mcg= msstv[k].mCbCg| msstv[k+1].mCbCg| msstv[k+2].mCbCg| msstv[k+3].mCbCg;
            sy = int(msstv[k].sY) | (int(msstv[k+1].sY) << 1) | (int(msstv[k+2].sY)<<2) | (int(msstv[k+3].sY)<<3);
            sdg= int(msstv[k].sYdDg) | (int(msstv[k+1].sYdDg) << 1) | (int(msstv[k+2].sYdDg)<<2) | (int(msstv[k+3].sYdDg)<<3);
            sco= int(msstv[k].sCrCo) | (int(msstv[k+1].sCrCo) << 1) | (int(msstv[k+2].sCrCo)<<2) | (int(msstv[k+3].sCrCo)<<3);
            scg= int(msstv[k].sCbCg) | (int(msstv[k+1].sCbCg) << 1) | (int(msstv[k+2].sCbCg)<<2) | (int(msstv[k+3].sCbCg)<<3);
